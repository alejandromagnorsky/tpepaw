...
public class Issue extends AbstractModel implements Comparable<Issue>, Progress {
	...
	@ManyToMany
	@JoinTable(name = "issue_followers", joinColumns = @JoinColumn(name = "issueid"), inverseJoinColumns = @JoinColumn(name = "userid"))
	@Sort(type = SortType.NATURAL)
	...
	public Issue(Project p, String title, String description,
		...
		if (issueListener == null)
			issueListener = new EmailNotifier();
		this.issueListener = issueListener;
		issueListener.onIssueCreate(this);
	}
	
	public void setAffectedVersions(User user, List<Version> affectedVersions){
		List<Version> toDelete = new ArrayList<Version>(this.affectedVersions);
		
		for (Version v : affectedVersions)
			toDelete.remove(v);
		
		for (Version v : toDelete){
			removeAffectedVersion(user, v);
			v.removeAffectingIssue(user, this);
		}
		
		for (Version v : affectedVersions){
			addAffectedVersion(user, v);
			v.addAffectingIssue(user, this);
		}
	}
	
	public void setFixedVersions(User user, List<Version> fixedVersions){
		List<Version> toDelete = new ArrayList<Version>(this.fixedVersions);
		
		for (Version v : fixedVersions)
			toDelete.remove(v);
		
		for (Version v : toDelete){
			removeFixedVersion(user, v);
			v.removeFixedIssue(user, this);
		}
		
		for (Version v : fixedVersions){
			addFixedVersion(user, v);
			v.addFixedIssue(user, this);
		}
	}
	...
	public void setAssignedUser(User source, User user) {
		if (user != null || assignedUser != null) {
			IssueLog log = new IssueLog(this, source, new DateTime(),
					ChangeType.Assigned, (assignedUser == null) ? null
							: assignedUser.getName(), (user == null) ? null : user.getName());
	...
	private void setState(User source, State state) {
		...
		if (issueListener == null)
			issueListener = new EmailNotifier();
		issueListener.onIssueStateChange(this, state);
		this.state = state;
	}
	...
	public void setEstimatedTime(User source, Time estimatedTime) {
		if(estimatedTime != null || this.estimatedTime != null) {
			TimeConverter c = new TimeConverter();
			IssueLog log = new IssueLog(this, source, new DateTime(),
					ChangeType.Estimated, c.convertToString(this.estimatedTime, null),
					c.convertToString(estimatedTime, null));
		}
	...
	public boolean canMark(User user, State state) {
		...
		case Ongoing:
			return getState().equals(State.Open) && getAssignedUser() != null
					&& (getAssignedUser().equals(user));
		case Completed:
			return getState().equals(State.Ongoing);
		case Closed:
	...
	public void setResolution(User source, Resolution resolution) {
		...
		IssueLog log = new IssueLog(this, source, new DateTime(),
				...
				this.resolution != null ? this.resolution.getCaption() : "",
	...
	public boolean filter(Filter filter) {
		if (!this.getProject().equals(filter.getProject())
				|| (filter.getIssueCode() != null && !this.getCode().contains(
						filter.getIssueCode()))
				|| (filter.getIssueTitle() != null && !this.getTitle()
						.contains(filter.getIssueTitle()))
				|| (filter.getIssueDescription() != null && this.getDescription() != null && !this
						.getDescription()
		...
		if (filter.getAffectedVersion() != null
				&& this.getAffectedVersions() != null
				&& !this.getAffectedVersions().contains(
						filter.getAffectedVersion()))
			return false;
	...
	public boolean canAddFollower(User user) {
		return user != null && !followers.contains(user)
				&& (project.getIsPublic() || project.isMember(user) || user.getAdmin());
	}

	public boolean canRemoveFollower(User user) {
		return user != null && followers.contains(user)
				&& (project.getIsPublic() || project.isMember(user) || user.getAdmin());
	}

	public void addFollower(User user) {
		if (!canAddFollower(user))
			throw new IllegalArgumentException();
		this.followers.add(user);
		user.followIssue(this);
	}

	public void removeFollower(User user) {
		if (!canRemoveFollower(user))
			throw new IllegalArgumentException();
		this.followers.remove(user);
		user.unfollowIssue(this);
	}
}
