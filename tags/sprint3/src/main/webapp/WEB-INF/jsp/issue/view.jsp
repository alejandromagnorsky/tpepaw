		...
		<div id="mainbar">
			<div class="mainbar-title">
				...
				<div class="mainbar-controls">
					<c:if test="${canRelateIssues}">
						<a href="../issue/relate?code=<c:out value="${issue.code}"></c:out>"><div class="relationsButton" title="Relacionar esta tarea"></div></a>
					</c:if>
					
					<c:if test="${canViewIssueFiles}">
						<a href="../issuefile/list?code=<c:out value="${issue.code}"></c:out>"><div class="fileListButton" title="Gestor de archivos adjuntos"></div></a>					
					</c:if>					
					<a href="../work/list?code=<c:out value="${issue.code}"></c:out>"><div class="stateButton" title="Ver trabajos"></div></a>
					<c:if test="${canAddCollaborator}"><a href="../issue/addCollaborator?code=<c:out value="${issue.code}"></c:out>"><div class="addUserButtonColor" title="Agregar colaborador"></div></a></c:if>
					<c:if test="${canRemoveCollaborator}"><a href="../issue/removeCollaborator?code=<c:out value="${issue.code}"></c:out>"><div class="removeUserButtonColor" title="Quitar colaborador"></div></a></c:if>
			...
			<c:if test="${issue.state.caption eq 'En curso'  }">
				...
				<div class="issue-view-left-column">
					...
					<span class="bold">Votos:  <c:out value="${issue.quantOfVotes}"></c:out></span><br />
					
					<span class="bold">Colaboradores: </span>
						<ul>
							<c:forEach var="user" items="${issue.collaborators}">
					...
					<c:if test="${issue.priority != null }">
						<span class="bold">Prioridad: </span> <c:out value="${issue.priority.caption}"></c:out><br />
					</c:if>
					
					<c:if test="${issue.type != null }">
						<span class="bold">Tipo: </span> <c:out value="${issue.type.caption}"></c:out><br />
					</c:if>
					
					<span class="bold">Versiones afectadas: </span>
					<ul>
						<c:forEach var="version" items="${affectedVersions}">
							<li><c:out value="${version.name}"/></li>
						</c:forEach>
					</ul>
					
					<span class="bold">Versiones resueltas: </span> 
					<ul>
						<c:forEach var="version" items="${fixedVersions}">
							<li><c:out value="${version.name}"/></li>
						</c:forEach>
					</ul>
				...
				<div class="issue-view-right-column">
					<span class="bold">Depende de:</span>
					<c:if test="${not empty dependsOnList}">
						<ul>
							<c:forEach var="issue" items="${dependsOnList}">
								<li><a href="../issue/view?code=<c:out value="${issue.code}"></c:out>"><c:out value="${issue.title}"></c:out></a></li>
							</c:forEach>
						</ul>
					</c:if>
					<c:if test="${empty dependsOnList}">
						<ul><span class="warning">Ninguna tarea.</span></ul>
					</c:if>
					
					<span class="bold">Es necesaria para:</span>
					<c:if test="${not empty necessaryForList}">
						<ul>
							<c:forEach var="issue" items="${necessaryForList}">
								<li><a href="../issue/view?code=<c:out value="${issue.code}"></c:out>"><c:out value="${issue.title}"></c:out></a></li>
							</c:forEach>
						</ul>
					</c:if>
					<c:if test="${empty necessaryForList}">
						<ul><span class="warning">Ninguna tarea.</span></ul>
					</c:if>
					
					<span class="bold">Se relaciona con:</span>
					<c:if test="${not empty relatedToList}">
						<ul>
							<c:forEach var="issue" items="${relatedToList}">
								<li><a href="../issue/view?code=<c:out value="${issue.code}"></c:out>"><c:out value="${issue.title}"></c:out></a></li>
							</c:forEach>
						</ul>
					</c:if>
					<c:if test="${empty relatedToList}">
						<ul><span class="warning">Ninguna tarea.</span></ul>
					</c:if>
					
					<span class="bold">Está duplicada con:</span>
					<c:if test="${not empty duplicatedWithList}">
						<ul>
							<c:forEach var="issue" items="${duplicatedWithList}">
								<li><a href="../issue/view?code=<c:out value="${issue.code}"></c:out>"><c:out value="${issue.title}"></c:out></a></li>
							</c:forEach>
						</ul>
					</c:if>
					<c:if test="${empty duplicatedWithList}">
						<ul><span class="warning">Ninguna tarea.</span></ul>
					</c:if>
				</div>
				</div>
				
				<span class="bold">Cambios: </span> <br />
					<div class="bold log-element">
						<span class="log-span">Usuario</span>
						<span class="log-span">Fecha</span>
						<span class="log-span">Tipo de cambio</span>
						<span class="log-span">Valor anterior</span>
						<span class="log-span">Valor nuevo</span>
					<br />
					</div>
				<div class="log-div">
					<c:forEach var="log" items="${issueLogs}">
						<div class="log-element">
							<span class="log-span"><c:out value="${log.source.name}"/></span>
							<span class="log-span"> <c:out value="${log.date.dayOfMonth }"></c:out>/<c:out value="${log.date.monthOfYear}"></c:out>/<c:out value="${log.date.year }"></c:out> </span>
							<span class="log-span"> <c:out value="${log.type.caption}"/></span>
							<span class="old-log-value log-span"> <c:out value="${log.previous}"/></span>
							<span class="new-log-value log-span"> <c:out value="${log.actual}"/></span>
						</div>
					</c:forEach>
				</div>
			...
			<div class="mainbar-title">
					<div class="mainbar-controls-right medium-size ">
						...
						<c:if test="${canVoteIssue}">
							<c:if test="${!voted}">
								<form class="controller-form"  method="post" action="../issue/vote?code=<c:out value="${issue.code}"></c:out>" >
									<input class="issue-controller medium-size" type="submit" value="Votar"/>
								</form>
							</c:if>
							<c:if test="${voted}">
								<form class="controller-form"  method="post" action="../issue/removeVote?code=<c:out value="${issue.code}"></c:out>" >
									<input class="issue-controller medium-size" type="submit" value="Remover voto"/>
								</form>
							</c:if>
						</c:if>
					</div>
			...
			<c:if test="${canAddComment }">
				<div class="issue-comments">
					...
					<form:form class="issue-comment-form" action="../issue/view" method="POST" commandName="commentForm" >
